...
var buildNextParameterSet = (attemptCount) => {
  ...
  const incrementFactor = Math.pow(1.1, completedBlockCount);
  const withIncrement = (defaultValue) => defaultValue * incrementFactor;
  return {
...
var handler = async (event) => {
  ...
  if (attemptCount <= 0 || attemptCount % RUNS_PER_BLOCK !== 0) {
    throw new Error(`attemptCount must be an exact multiple of ${RUNS_PER_BLOCK}`);
  }
