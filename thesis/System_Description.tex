...
Rather than being triggered indirectly through DynamoDB Streams, the machine-learning backend is invoked synchronously as part of the GraphQL request flow. Whenever a participant finishes a block of 10 trials, the frontend calls the triggerNextParameterSet mutation, whose AppSync resolver invokes an AWS Lambda function. This Lambda function reads the calling participant's full trial history from DynamoDB, additionally performs a table scan to collect the anonymized trial histories of all other participants recorded so far, and forwards both as a single JSON payload to an Amazon SageMaker inference endpoint. SageMaker is configured as a serverless endpoint with a maximum concurrency of 1 and 3072\,MB of memory, so that compute resources and costs are only allocated for the brief duration of an actual invocation instead of running continuously between experimental sessions. The endpoint executes AWS's managed, prebuilt SageMaker Scikit-learn inference container, into which only a small custom inference script, packaged as model artifact. The parameter set produced by the model is returned to the Lambda function, written back into DynamoDB as the participant's nextParameterSet, and subsequently retrieved by the frontend once the participant confirms that they are ready to continue with the next block of trials.
...
Scrolling is confined to the valid range spanned by the list. When a gesture or a fling would move the content beyond the top or bottom edge in Android, overscrolling is permitted only up to a fixed maximum overscroll distance of $120$\,px and is visually compressed by a fixed resistance factor of $0.35$, so that only $35\,\%$ of the overflow is translated into visible motion. Once the touch is released in an overscrolled state, or once a fling reaches an edge, the content is returned to the nearest valid bound. This edge handling is independent of the transfer-function parameters and corresponds in spirit to the ballistic and spring-back phases.Those are not implemented in this Work.
...
Only after these first three blocks have been completed does the system switch to model-based parameter generation. At that point, the optimization backend treats the problem as participant-aware, pooled, and multi-objective rather than as a single-participant regression problem. Whenever a new parameter set is requested, the Lambda function retrieves the full per-participant trial histories from DynamoDB, including the requesting participant as well as all previously recorded participants, and forwards the aggregated payload to the SageMaker endpoint.

The request payload is prepared in a structured form that reflects the actual data pipeline of the optimizer. Each trial record contains the measured completion time \texttt{timeMs}, the target index \texttt{targetNumber}, the block index \texttt{blockIndex}, and the parameter set that was active for that block in \texttt{paperParams}. On the inference side, the records are first coerced into a canonical schema, invalid values are replaced by the defaults of the transfer function, and the parameters are validated against the admissible bounds. The trials are then grouped by block, each block is assigned the parameter vector from its first entry, and the ten target positions are summarized by their mean completion time per target. This yields a per-block observation with a three-dimensional parameter vector and a ten-dimensional response vector, one scalar objective per target position. If a target number is missing from a block, the corresponding value is filled with the block-wise mean completion time as a fallback, so that every block contributes a complete target vector to the model.

The resulting training data are then assembled as a pooled, participant-aware table of the form
\begin{equation}
    [\text{scrollFriction},\ \text{decelerationRate},\ \text{inflexion},\ \text{participantId}] \rightarrow [t_{30}, t_{60}, \ldots, t_{300}],
\end{equation}
where each participant contributes one task dimension encoded as an additional numeric feature. Duplicate or near-identical rows are collapsed by rounding to six decimal places and averaging the associated objective vectors, which prevents a small number of repeated evaluations from making the Gaussian-process covariance numerically unstable. The optimizer then fits a multi-objective Bayesian model using BoTorch's \texttt{qLogNoisyExpectedHypervolumeImprovement} acquisition function. A separate Gaussian process is trained for each of the ten target-position objectives, with the shared task feature included as part of the input space and the negative completion times used as the objective to be maximized. This keeps the optimization participant-aware without requiring a separate model per participant: the model can borrow statistical strength across users while still allowing explicit task-specific differences to be learned through the task feature's own length scale.
