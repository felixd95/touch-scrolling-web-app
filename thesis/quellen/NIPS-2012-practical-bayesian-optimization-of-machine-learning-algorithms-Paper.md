# NIPS-2012-practical-bayesian-optimization-of-machine-learning-algorithms-Paper

Quelle: C:\Users\Felix\Desktop\Master\Thesis\Quellen\ML\NIPS-2012-practical-bayesian-optimization-of-machine-learning-algorithms-Paper.pdf
...
mum of a function f(x) on some bounded setX , which we will take to be a subset of RD. What
...
the function, while integrating out uncertainty. The essential philosophy is to useall of the informa-
tion available from previous evaluations off(x) and not simply rely on local gradient and Hessian
...
will take here to be of the formf :X→ R. The GP is deﬁned by the property that any ﬁnite set ofN
points{xn∈X} N
...
n=1, where yn∼N (f(xn),ν ) andν is the variance of noise intro-
...
evaluated next via a proxy optimization xnext = argmaxxa(x), where several different functions
...
variance function σ2(x ;{xn,yn},θ ). In the proceeding, we will denote the best current value
as xbest = argminxnf(xn) and the cumulative distribution function of the standard normal asΦ(·).
...
with a tunableκ to balance exploitation against exploration.
...
amplitudeθ0, the observation noiseν, and a constant mean m. The most commonly advocated ap-
...
n=1,θ,ν,m ) =N (y|m1, Σθ +νI), where y = [y1,y 2,··· ,yN]T,
and Σθ is the covariance matrix resulting from theN input points under the hyperparametersθ.
...
n=1) dθ, (6)
wherea(x) depends on θ and all of the observations. For probability of improvement and EI, this
...
anN-dimensional linear system (and our function evaluations are assumed to be much more expen-
...
tionc(x) :X→ R+. We can nevertheless employ our Gaussian process machinery to modellnc(x)
alongsidef(x). In this work, we assume that these functions are independent of each other, although
...
n=1, and in
whichJ evaluations are pending at locations{xj}J
...
j=1,{xn,yn}N
n=1) dy1··· dyJ. (7)
This is simply the expectation ofa(x) under aJ-dimensional Gaussian distribution, whose mean and
...
In this section, we empirically analyse 1 the algorithms introduced in this paper and compare to ex-
...
sion of minxnf(xn) over the number of function evaluations or time, averaged over multiple runs
of each algorithm. If not speciﬁed otherwise, xnext = argmaxxa(x) is computed using gradient-
...
We ﬁrst compare to standard approaches and the recent Tree Parzen Algorithm 2 (TPA) of Bergstra
...
optimization techniques [2] that is deﬁned overx∈ R2 where 0≤x1≤ 15 and−5≤x2≤ 15. We
...
on a log scale from 0 to 1, the 𝓁2 regularization parameter, between 0 and 1, the mini batch size,
...
andη =α = 0.01 in our experiments in order to emulate their analysis and repeated exactly the grid
...
parameterized byα, which enables the model to outperform latent structured SVMs. This additional
...
quences. We explore 25 settings of the parameterC, on a log scale from 10−1 to 106, 14 settings of
α, on a log scale from 0.1 to 5 and the model convergence tolerance, ϵ∈{ 10−4,10−3,10−2,10−1}.
...
volutional network [22] on the CIFAR-10 benchmark dataset using the code provided5. This model
...
sian process based global optimization. http://hal.archives-ouvertes.fr/
hal-00507632, 2010.
...
On random weights and unsupervised feature learning. InProceedings of the 28th International
